...
def sync_sms_to_qdrant(client, qdrant, records, batch_size=100):
    ...
    if not new_records:
        logging.info("No new SMS found to add.")
        return
    ...
    for batch_records in chunked(new_records, batch_size):
        ...
        logging.info(f"Upserted batch of {len(points_to_upsert)}")
...
def search_similar_sms(client, qdrant, query_text):
    ...
    for res in search_result:
        logging.info(f"Found: '{res.payload['clean']}' (Label: {res.payload['label']}, Score: {res.score:.3f})")
